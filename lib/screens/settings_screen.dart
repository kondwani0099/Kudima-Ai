...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget buildUserProfile() {
    return Column(
      children: [
        ...
        //       : AssetImage('assets/profile.jpeg'), // Replace with your user profile image
        // ),
         CircleAvatar(
        radius: 70.0,
        ...
        Text(
          user?.displayName ?? 'John Doe', // Replace with user's name
          style: TextStyle(
            fontSize: 20.0,
        ...
        Text(
          user?.email ?? 'johndoe@example.com', // Replace with user's email
          style: TextStyle(
            fontSize: 16.0,
          ),
        ),
  ...
  Future<void> _deleteAccount() async {
    try {
      await user?.delete();
      // After deleting the account, navigate to the login or sign-up screen.
      Navigator.of(context).pushReplacementNamed('/loginn');
    } catch (e) {
      print('Account Deletion Error: $e');
      // Handle the error, you can show an error message or alert dialog to the user.
    }
  }
}
